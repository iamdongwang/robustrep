"""Fetch one evidence URI (http/ipfs/data) safely, or refuse it.

This module owns the *single-URI* security boundary only; the batch layer that
walks every pending URI, classifies it and bounds the run's tx-hash lookups
lives in `evidence_batch`, which imports `http_fetch_text` from here (the
...
def _fetch_url(url: str, session, resolver: Resolver, timeout, redirects_left: int = MAX_REDIRECTS
    ...
    A caller-supplied session is never closed here -- ``classify_all`` (in
    ``evidence_batch``) reuses one per worker thread across many URIs and
    closes them itself.
    """
